...
{
	printf("usage: pdsasm [-d <data-segment-size>] [-t] [-s] <infile> <outfile>");
}
...
static char *_strrchr(register const char *s, int c)
{
	char *rtnval = 0;
...
*****************************************************************************/
{
	...
	g_psIdentifierList[uId].eType = PDSASM_IDENTIFIER_TYPE_NONE;
	g_psIdentifierList[uId].pszName = _strdup(pszName);
...
*****************************************************************************/
{
	...
	*/
	pszUCaseIdentifierName = _strdup(pszIdentifierName);
...
int main(int argc, char* argv[])
{
	...
	IMG_PCHAR				pszOutDefineName;
	IMG_BOOL				bDisasmMode = IMG_FALSE;
	...
	while (argc > 1)
	{
		...
		{
			bDisasmMode = IMG_TRUE;
			uDataSegmentSize = atoi(argv[2]);

			argc--;
			argv++;
		}
		else if (_stricmp(argv[1], "-cpp") == 0)
		{
	...
	{
		g_sCurrentLocation.pszSourceFile = _strdup("stdin");
		yyin = fInFile = stdin;
#if !defined(LINUX)
	...
	{
		g_sCurrentLocation.pszSourceFile = _strdup(argv[1]);
	...
	{
		pszOutFileName = _strdup(argv[2]);
	}

	if (argc == 4)
	{
		pszOutArrayName = _strdup(argv[3]);	
	}
	else if (argc == 3)
	{
		...
		pszOutArrayName = _strdup(pszStart);
		if ((pszDot = _strrchr(pszOutArrayName, '.')) != NULL)
		{
			*pszDot = '\0';
		}
	}
	...
	*/
	{
		...
		pszOutDefineName = _strdup(pszOutArrayName);
	...
	if (bDisasmMode)
	{
		...
		uEncodedProgramLength = ftell(fInFile);
		fseek(fInFile, 0, SEEK_SET);

		pui8EncodedProgram = malloc(uEncodedProgramLength);
		fread(pui8EncodedProgram, sizeof(IMG_UINT8), uEncodedProgramLength, fInFile);
		fclose(fInFile);

		printf("Encoded Program Size = 0x%X\nData Segment Size = 0x%X\n", uEncodedProgramLength, uDataSegmentSize);
